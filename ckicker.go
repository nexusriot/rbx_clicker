...
var (
	clicking int32 // atomic: 0=stopped 1=clicking
	cps      int32 = 2
)
...
func main() {
	...
	if err := registerHotkey(ID_HOTKEY_START, MOD_NOREPEAT, VK_F6); err != nil {
		fmt.Printf("Failed to register START hotkey: %v\n", err)
	}
	if err := registerHotkey(ID_HOTKEY_STOP, MOD_NOREPEAT, VK_F7); err != nil {
	...
	fmt.Println("Ready.")
	fmt.Println("  F6   START clicking")
	fmt.Println("  F7   STOP clicking")
	...
	go func() {
		for {
			if atomic.LoadInt32(&clicking) == 1 {
				click()
				time.Sleep(time.Second / time.Duration(atomic.LoadInt32(&cps)))
			} else {
